...
Image* CommandProcessor:: getImage(const char* filename)
{
    ...
    if (!image)
        throw std::logic_error("Invalid extention!\n");
...
void CommandProcessor::saveas(const myString& filename)const
{
    ...
    {
        SharedPtr<Image> image = sessionHolder.getSession(0)->getImageAtIndex(0);
...
void CommandProcessor::grayscale(int sessionId) {
    if (sessionHolder.sessionExists(sessionId)) {
        ...
        GrayscaleConverter grayscaleConverter;
            ...
            SharedPtr<ImageCommand> command(new GrayscaleCommand(&grayscaleConverter));
            session->addTransformation(command); //in add we also apply the transformations
    }
    else 
...
void CommandProcessor::monochrome(int sessionId) {
    if (sessionHolder.sessionExists(sessionId)) {
        ...
        MonochromeConverter monochromeConverter;
            ...
            SharedPtr<ImageCommand> command(new MonochromeCommand(&monochromeConverter));
            session->addTransformation(command);
    }
    else 
        throw std::logic_error("Session does't exist!\n");
}
void CommandProcessor::negative(int sessionId)
{
    if (sessionHolder.sessionExists(sessionId)) {
        ...
        NegativeConverter negativeConverter;
            ...
            SharedPtr<ImageCommand> command(new NegativeCommand(&negativeConverter));
            session->addTransformation(command);
    }
    else
        throw std::logic_error("Session does't exist!\n");
...
void CommandProcessor::rotate(const myString& direction, int sessionId)
{
    if (sessionHolder.sessionExists(sessionId)) {
        ...
        int imagesInSession = session->getNumImages();
        Rotator rotator;
        SharedPtr<ImageCommand> command(new RotateCommand(&rotator, direction));
        session->addTransformation(command);
    }
    else 
        throw std::logic_error("Session does't exist!\n");
...
void CommandProcessor::collage(const myString& dir, int sesId, const myString& firstFile, const myString& secFile, const myString& OutputFile)
{
    if (sessionHolder.sessionExists(sesId))
    {
        ...
        if (img1 && img2)
        {
            SharedPtr<Image> collage;
            if (dir == "horizontal")
            {
                collage = Collage::horizontalCollage(SharedPtr<Image>(img1), SharedPtr<Image>(img2));
            }
            else if (dir == "vertical")
            {
                collage = Collage::verticalCollage(SharedPtr<Image>(img1), SharedPtr<Image>(img2));
            }
            else
            {
                throw std::logic_error("Invald direction!\n");
            }

            myString collageFilename(OutputFile);

            sessionHolder.getSession(activeSessionId)->saveImage(collage, collageFilename);
        }
        else
